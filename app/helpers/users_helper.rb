module UsersHelper
  def bookmarklet_url user
    if Rails.env == 'production'
      ...
      "javascript:(function(){window.reading={token:'#{user.token}'};var head=document.getElementsByTagName('head')[0],script=document.createElement('script');script.src='http://#{DOMAIN}/assets/bookmarklet/loader.js';head.appendChild(script);})()"
    end
  end
end
